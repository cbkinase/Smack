	...
	if (Object.values(channel.Members).length === 2) {
		...
		introTitle = (
			<div className="dm-intro-title-container">
				<img
					alt=""
					className="dm-intro-picture"
					src={member.avatar}
				></img>
				<h3 onClick={nameClickFn} id="dm-intro-title">
					{channelName}
				</h3>
				<ActivityStatus
					user={member}
		...
		channelIntroAbout = (
			<p style={{ paddingTop: "13px" }}>
				...
				<span onClick={nameClickFn} id="intro-owner-name">
		...
		channelIntroBtn = (
			<button
				onClick={nameClickFn}
				className="channel-intro-btn channel-intro-btn-dm"
			>
				View Profile
			</button>
		);
